...
<p class="actions"><?php if (count($items) < 50) echo $this->Html->link("Artikel hinzufügen", array("controller" => "items", "action" => "create", $seller["Seller"]["id"])); ?></p>
<table>
	<tr>
		<th>Nummer</th>
		<th>Beschreibung</th>
	...
	<tr>
		...
		<td class="actions"><?php echo $this->Html->link("Bearbeiten", array("controller" => "items", "action" => "update", $item["Item"]['id'])); ?>
			<?php echo $this->Form->postLink("Löschen", array("controller" => "items", "action" => "delete", $item["Item"]['id']),
					array("confirm" => "Sind Sie sicher, dass Sie diesen Artikel löschen wollen?")); ?></td>
	</tr>
	<?php endfor; ?>
	<?php unset($item); ?>
</table>
<?php if (count($items) > 0): ?>
<p>Sobald Sie alle Artikel angelegt und eine Reservierungsnummer erhalten haben, können Sie die Etiketten für die Artikel erzeugen und drucken.</p>
	<?php if ($reservation): ?>
	<p class="actions"><?php echo $this->Html->link("Etiketten erzeugen und herunterladen",
			array("controller" => "items", "action" => "label", $reservation["Reservation"]["id"]),
			array(), "Sobald Sie die Etiketten erzeugen, werden alle bis jetzt eingegebenen Artikel gesperrt. " .
			"Sie können diese Artikel nicht mehr bearbeiten. Sie können jedoch danach noch weitere Artikel hinzufügen. " .
			"Wollen Sie die Etiketten erzeugen lassen und damit die aktuellen Artikel sperren?"); ?></p>
